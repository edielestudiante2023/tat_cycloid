...
<div class="page-header">
    <h1><i class="fas fa-broom me-2"></i> Dotación Aseadora</h1>
    <a href="<?= base_url('client/inspecciones/dotacion-aseadora') ?>" class="btn-back">
...
<div class="card mb-3">
    <div class="card-body">
        ...
        <table class="table table-sm mb-0" style="font-size:14px;">
            ...
            <?php if (!empty($inspeccion['nombre_cargo'])): ?><tr><td class="text-muted">Nombre / Cargo</td><td><?= esc($inspeccion['nombre_cargo']) ?></td></tr><?php endif; ?>
            <?php if (!empty($inspeccion['actividades_frecuentes'])): ?><tr><td class="text-muted">Actividades</td><td><?= nl2br(esc($inspeccion['actividades_frecuentes'])) ?></td></tr><?php endif; ?>
        </table>
    </div>
</div>
...
<div class="card mb-3">
    <div class="card-body">
        ...
        <div class="row g-2">
            ...
            <div class="col-6"><small class="text-muted d-block mb-1">Cuerpo completo</small><img src="<?= base_url($inspeccion['foto_cuerpo_completo']) ?>" class="img-fluid rounded" style="max-height:200px; object-fit:cover; cursor:pointer; border:1px solid #ddd; width:100%;" onclick="openPhoto(this.src)"></div>
            <?php endif; ?>
            <?php if (!empty($inspeccion['foto_cuarto_almacenamiento'])): ?>
            <div class="col-6"><small class="text-muted d-block mb-1">Cuarto almacenamiento</small><img src="<?= base_url($inspeccion['foto_cuarto_almacenamiento']) ?>" class="img-fluid rounded" style="max-height:200px; object-fit:cover; cursor:pointer; border:1px solid #ddd; width:100%;" onclick="openPhoto(this.src)"></div>
            <?php endif; ?>
        </div>
    </div>
</div>
...
<div class="card mb-3">
    <div class="card-body">
        <h6 style="font-size:14px; color:#999; font-weight:700; margin-bottom:12px;"><i class="fas fa-vest me-1"></i> ESTADO DE DOTACIÓN EPP</h6>
...
<div class="card mb-3"><div class="card-body"><h6 style="font-size:14px; color:#999; font-weight:700; margin-bottom:12px;"><i class="fas fa-clipboard-check me-1"></i> CONCEPTO FINAL</h6><p style="font-size:14px; margin:0;"><?= nl2br(esc($inspeccion['concepto_final'])) ?></p></div></div>
<?php endif; ?>

<?php if (!empty($inspeccion['observaciones'])): ?>
<div class="card mb-3"><div class="card-body"><h6 style="font-size:14px; color:#999; font-weight:700; margin-bottom:12px;"><i class="fas fa-comment-alt me-1"></i> OBSERVACIONES</h6><p style="font-size:14px; margin:0;"><?= nl2br(esc($inspeccion['observaciones'])) ?></p></div></div>
<?php endif; ?>
...
<div class="mb-4">
    ...
    <a href="<?= base_url('/inspecciones/dotacion-aseadora/pdf/') ?><?= $inspeccion['id'] ?>" class="btn btn-primary" target="_blank" style="background:#e76f51; border-color:#e76f51;"><i class="fas fa-file-pdf"></i> Ver PDF</a>
    <?php endif; ?>
</div>
