...
<div class="page-header">
    <h1><i class="fas fa-clipboard-check me-2"></i> Contaminación Cruzada</h1>
...
<div class="card mb-3" style="border-left:4px solid <?= $ok ? '#198754' : '#dc3545' ?>;">
    <div class="card-body">
        ...
        <?php if (!empty($header['observaciones_generales'])): ?>
            <hr>
            <div><strong>Observaciones:</strong> <?= nl2br(esc($header['observaciones_generales'])) ?></div>
        <?php endif; ?>
    </div>
</div>
...
<?php foreach ($detalles as $d):
    ...
    if ($d['estado'] === 'cumple')     { $borde = '#198754'; $bClass = 'bg-success'; $bText = 'Cumple'; }
    elseif ($d['estado'] === 'no_cumple'){ $borde = '#dc3545'; $bClass = 'bg-danger';  $bText = 'No cumple'; }
?>
...
echo view('client/inspecciones/layout', ['title' => 'Contaminación Cruzada · ' . date('d/m/Y', strtotime($header['fecha_hora'])), 'content' => $content]);