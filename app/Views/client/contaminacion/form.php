...
<div class="page-header">
    <h1><i class="fas fa-exchange-alt me-2"></i> Nueva inspección Contaminación Cruzada</h1>
    <a href="<?= base_url('client/contaminacion') ?>" class="btn-back">
...
<form id="formContaminacion" action="<?= base_url('client/contaminacion/guardar') ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="card mb-2">
            <div class="card-body py-3">
                ...
                <div class="btn-group w-100 mb-2" role="group">
                    <input type="radio" class="btn-check" name="estado[<?= (int)$it['id_item'] ?>]" id="e_<?= $it['id_item'] ?>_ok" value="cumple" autocomplete="off">
                    <label class="btn btn-outline-success" for="e_<?= $it['id_item'] ?>_ok"><i class="fas fa-check"></i> Cumple</label>

                    <input type="radio" class="btn-check" name="estado[<?= (int)$it['id_item'] ?>]" id="e_<?= $it['id_item'] ?>_bad" value="no_cumple" autocomplete="off">
                    <label class="btn btn-outline-danger" for="e_<?= $it['id_item'] ?>_bad"><i class="fas fa-times"></i> No cumple</label>

                    <input type="radio" class="btn-check" name="estado[<?= (int)$it['id_item'] ?>]" id="e_<?= $it['id_item'] ?>_na" value="no_aplica" autocomplete="off">
                    <label class="btn btn-outline-secondary" for="e_<?= $it['id_item'] ?>_na"><i class="fas fa-ban"></i> No aplica</label>
                </div>
                ...
                <div class="row g-2">
                    <div class="col-md-6">
                        ...
                        <input type="file" name="foto_<?= (int)$it['id_item'] ?>" class="form-control form-control-sm"
    ...
    <div class="card mb-3">
        <div class="card-body">
            ...
            <textarea name="observaciones_generales" rows="3" class="form-control"
                      placeholder="Notas generales, acciones correctivas tomadas, etc."></textarea>
        </div>
    </div>
...
document.getElementById('formContaminacion').addEventListener('submit', function(e) {
...
document.querySelectorAll('[name^="estado["]').forEach(function(radio) {
    radio.addEventListener('change', function() {
        ...
        if (this.value === 'no_aplica') {
            ...
            label.innerHTML = '<i class="fas fa-camera me-1"></i> Foto <span class="text-muted">(no aplica)</span>';
        } else {
...
echo view('client/inspecciones/layout', ['title' => 'Nueva inspección Contaminación Cruzada', 'content' => $content]);