...
<div class="page-header">
    <h1><i class="fas fa-exchange-alt me-2"></i> Contaminación Cruzada</h1>
    <a href="<?= base_url('client/contaminacion/nueva') ?>" class="btn-back" style="background:rgba(255,255,255,.25);">
...
<?php if (empty($inspecciones)): ?>
    <div class="card p-4 text-center">
        <i class="fas fa-exchange-alt" style="font-size:48px; color:#ccc;"></i>
        <h5 class="mt-3 text-muted">Sin inspecciones aún</h5>
        <p class="text-muted">Registra la primera inspección POES 4.2 de contaminación cruzada.</p>
        <a href="<?= base_url('client/contaminacion/nueva') ?>" class="btn mt-2" style="background:#ee6c21; color:#fff;">
                ...
                <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                    <div>
                        <h6 class="mb-1" style="color:#c9541a;">
                            <i class="fas fa-calendar-check me-1"></i> <?= date('d/m/Y H:i', strtotime($i['fecha_hora'])) ?>
                            <?php if ($ok): ?>
                        ...
                        <div class="text-muted small">
                            ...
                            <?php if ((int)$i['total_incumplen'] > 0): ?>
                                · <span class="text-danger"><strong><?= (int)$i['total_incumplen'] ?></strong> no cumplen</span>
                            <?php endif; ?>
                            ...
                            <div class="mt-2 d-flex gap-1 flex-wrap">
                                <?php foreach ($i['thumbs'] as $th): ?>
                                    <a href="<?= base_url('uploads/' . $th['foto']) ?>" target="_blank">
                                        <img src="<?= base_url('uploads/' . $th['foto']) ?>" alt=""
                    ...
                    <div class="d-flex gap-1">
                        <a href="<?= base_url('client/contaminacion/' . $i['id'] . '/ver') ?>"
                           class="btn btn-sm btn-outline-primary" style="border-color:#ee6c21; color:#ee6c21;">
                            <i class="fas fa-eye me-1"></i> Ver
                        </a>
                        <button type="button" class="btn btn-sm btn-outline-danger"
                                data-anular-url="<?= base_url('client/contaminacion/' . $i['id'] . '/eliminar') ?>"
                                data-anular-titulo="Inspección de contaminación cruzada del <?= esc($i['fecha_hora'] ?? $i['creado_en'] ?? '') ?>">
...
echo view('client/inspecciones/layout', ['title' => 'Contaminación Cruzada', 'content' => $content]);
