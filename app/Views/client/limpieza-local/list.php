...
<div class="page-header">
    <h1><i class="fas fa-broom me-2"></i> Inspección de Aseo</h1>
    <a href="<?= base_url('client/limpieza-local/nueva') ?>" class="btn-back" style="background:rgba(255,255,255,.25);">
...
<?php if (empty($inspecciones)): ?>
    <div class="card p-4 text-center">
        <i class="fas fa-broom" style="font-size:48px; color:#ccc;"></i>
        <h5 class="mt-3 text-muted">Sin inspecciones aún</h5>
        <p class="text-muted">Realiza la primera inspección de aseo del local.</p>
        <a href="<?= base_url('client/limpieza-local/nueva') ?>" class="btn mt-2" style="background:#ee6c21; color:#fff;">
                ...
                <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                    <div>
                        ...
                        <div class="text-muted small">
                            ...
                            <?php if ((int)$i['total_sucios'] > 0): ?>
                                · <span class="text-danger"><strong><?= (int)$i['total_sucios'] ?></strong> sucios</span>
                            <?php endif; ?>
                            ...
                            <div class="mt-2 d-flex gap-1 flex-wrap">
                                <?php foreach ($i['thumbs'] as $th): ?>
                                    <a href="<?= base_url('uploads/' . $th['foto']) ?>" target="_blank">
                                        <img src="<?= base_url('uploads/' . $th['foto']) ?>"
                                             alt="evidencia"
                    ...
                    <div class="d-flex gap-1">
                        <a href="<?= base_url('client/limpieza-local/' . $i['id'] . '/ver') ?>"
                           class="btn btn-sm btn-outline-primary" style="border-color:#ee6c21; color:#ee6c21;">
                            <i class="fas fa-eye me-1"></i> Ver
                        </a>
                        <button type="button" class="btn btn-sm btn-outline-danger"
                                data-anular-url="<?= base_url('client/limpieza-local/' . $i['id'] . '/eliminar') ?>"
                                data-anular-titulo="Inspección de limpieza del <?= esc($i['fecha_hora'] ?? $i['creado_en'] ?? '') ?>">
