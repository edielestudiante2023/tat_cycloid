...
<head>
    ...
    <title>Catálogo — Items — Almacenamiento</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        body { background: #f5f7fa; min-height: 100vh; }
        .page-wrap { max-width: 1100px; margin: 40px auto; padding: 0 16px; }
        .header-card { background: linear-gradient(135deg,#7c3aed,#a855f7); color:#fff; padding:24px; border-radius:14px; }
    </style>
</head>
<body>
<div class="page-wrap">
    <div class="header-card mb-4 d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div>
            <h3 class="mb-0"><i class="fas fa-boxes-stacked me-2"></i> Items — Almacenamiento</h3>
            <small>Items que aparecen en el checklist de Almacenamiento.</small>
        </div>
        <div>
            <a href="<?= base_url('admin/almacenamiento-items/nuevo') ?>" class="btn" style="background:#ee6c21;color:#fff;">
    ...
    <div class="mb-3 d-flex gap-2 align-items-center flex-wrap">
        ...
        <select class="form-select" style="max-width:320px;"
                onchange="if(this.value) window.location='<?= base_url('admin/almacenamiento-items/asignar/') ?>'+this.value;">
            ...
            <table class="table table-hover mb-0 align-middle">
                ...
                <?php foreach ($items as $i): ?>
                    <tr style="<?= (int)$i['activo'] === 0 ? 'opacity:0.55;' : '' ?>">
                        ...
                        <td>
                            <a href="<?= base_url('admin/almacenamiento-items/' . $i['id_item'] . '/editar') ?>"
                               class="btn btn-sm btn-outline-secondary">
                                <i class="fas fa-pen"></i>
                            </a>
                            <?php if ((int)$i['activo'] === 1): ?>
                                <form method="post" action="<?= base_url('admin/almacenamiento-items/' . $i['id_item'] . '/eliminar') ?>"
                                      onsubmit="return confirm('¿Desactivar este item?');" style="display:inline;">
                                    <button class="btn btn-sm btn-outline-danger"><i class="fas fa-trash"></i></button>
                                </form>
                            <?php else: ?>
                                <form method="post" action="<?= base_url('admin/almacenamiento-items/' . $i['id_item'] . '/activar') ?>" style="display:inline;">
