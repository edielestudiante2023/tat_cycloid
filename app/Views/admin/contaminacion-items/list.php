...
<head>
    ...
    <title>Catálogo — Items — Contaminación Cruzada</title>
...
<div class="page-wrap">
    <div class="header-card mb-4 d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div>
            <h3 class="mb-0"><i class="fas fa-exchange-alt me-2"></i> Items — Contaminación Cruzada</h3>
            <small>Items que aparecen en el checklist de Contaminación.</small>
        </div>
        <div>
            <a href="<?= base_url('admin/contaminacion-items/nuevo') ?>" class="btn" style="background:#ee6c21;color:#fff;">
    ...
    <div class="mb-3 d-flex gap-2 align-items-center flex-wrap">
        ...
        <select class="form-select" style="max-width:320px;"
                onchange="if(this.value) window.location='<?= base_url('admin/contaminacion-items/asignar/') ?>'+this.value;">
            ...
            <table class="table table-hover mb-0 align-middle">
                ...
                <?php foreach ($items as $i): ?>
                    <tr style="<?= (int)$i['activo'] === 0 ? 'opacity:0.55;' : '' ?>">
                        ...
                        <td>
                            <a href="<?= base_url('admin/contaminacion-items/' . $i['id_item'] . '/editar') ?>"
                               class="btn btn-sm btn-outline-secondary">
                                <i class="fas fa-pen"></i>
                            </a>
                            <?php if ((int)$i['activo'] === 1): ?>
                                <form method="post" action="<?= base_url('admin/contaminacion-items/' . $i['id_item'] . '/eliminar') ?>"
                                      onsubmit="return confirm('¿Desactivar este item?');" style="display:inline;">
                                    <button class="btn btn-sm btn-outline-danger"><i class="fas fa-trash"></i></button>
                                </form>
                            <?php else: ?>
                                <form method="post" action="<?= base_url('admin/contaminacion-items/' . $i['id_item'] . '/activar') ?>" style="display:inline;">
