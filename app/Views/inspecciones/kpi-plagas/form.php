...
<form method="post" action="<?= $action ?>" enctype="multipart/form-data" id="kpiPlagasForm">
    <?= csrf_field() ?>
    ...
    <div class="card mb-3">
        <div class="card-header bg-dark text-white py-2 px-3" style="font-size:13px;">
            <i class="fas fa-chart-pie me-1"></i> Indicador de Gestión
        </div>
        <div class="card-body p-3">
            <div class="mb-3">
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        btnFinalizar.addEventListener('click', function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = '<?= base_url('/inspecciones/' . esc($slug ?? 'kpi-plagas') . '/finalizar-grupo/' . ($inspeccion['id'] ?? '')) ?>';
                }
            });
        });
    }
    ...
    // ============================================================
    const STORAGE_KEY = 'kpi_plagas_draft_<?= $inspeccion['id'] ?? 'new' ?>';
    ...
    setTimeout(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed && currentId) {
                window.location.href = '<?= base_url('/inspecciones/' . ($slug ?? 'kpi-plagas') . '/finalizar-grupo/') ?>' + currentId;
            } else if (result.isDenied) {
                window.location.href = '<?= base_url('/inspecciones/' . ($slug ?? 'kpi-plagas') . '/create/') ?>' + savedCliente;
            }
        });
    }, 500);
})();
