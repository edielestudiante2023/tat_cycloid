...
<div class="container-fluid px-3 pb-5">
    ...
    <div class="card mb-3" id="formCard">
        <div class="card-body">
            <h6 class="card-title mb-3" style="font-size:14px; color:#c9541a; font-weight:700;">
...
function guardarAsistente() {
    ...
    if (faltantes.length > 0) {
        Swal.fire({ icon: 'warning', title: 'Campos obligatorios', html: 'Faltan: <strong>' + faltantes.join(', ') + '</strong>', confirmButtonColor: '#ee6c21' });
        return;
    }
    ...
    fetch(STORE_ASISTENTE_URL, { method: 'POST', body: formData })
        ...
        .then(data => {
            ...
            if (!data.success) {
                Swal.fire({ icon: 'error', title: 'Error', text: data.error || 'No se pudo guardar', confirmButtonColor: '#ee6c21' });
                btn.disabled = false;
        ...
        .catch(async () => {
            ...
            try {
                ...
                Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudo guardar la firma. Intente de nuevo.', confirmButtonColor: '#ee6c21' });
            }
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-check me-1"></i> Guardar y pasar al siguiente';
        });
...
function iniciarFinalizacion() {
    ...
    if (total === 0) {
        Swal.fire({ icon: 'warning', title: 'Sin asistentes', text: 'Debe registrar al menos un asistente.', confirmButtonColor: '#ee6c21' });
        return;
    }
...
async function syncManual() {
    ...
    try {
        ...
        if (result.synced > 0) {
            ...
            Swal.fire({ icon: 'warning', title: 'Sin conexion', text: 'Aun no hay internet. Se reintentara automaticamente.', confirmButtonColor: '#ee6c21' });
        }
    } catch (e) {
        Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudo sincronizar.', confirmButtonColor: '#ee6c21' });
    }
    btn.disabled = false;
