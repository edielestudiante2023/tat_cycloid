...
$action           = $isEdit ? base_url('/inspecciones/contingencia-agua/update/') . $inspeccion['id'] : base_url('/inspecciones/contingencia-agua/store');
$storageKey       = $isEdit ? 'cont_agua_draft_' . $inspeccion['id'] : 'cont_agua_draft_new';
$empresaCarrotanque = $isEdit ? ($inspeccion['empresa_carrotanque'] ?? '') : '';
$capacidadReserva   = $isEdit ? ($inspeccion['capacidad_reserva'] ?? '') : '';
?>

<h5 class="mb-3">
    <i class="fas fa-tint-slash me-2"></i>
    <?= $isEdit ? 'Editar' : 'Nuevo' ?> Plan de Contingencia — Sin Suministro de Agua
</h5>
...
<form id="aguaForm" action="<?= $action ?>" method="post">
    ...
    <!-- PROVEEDOR AGUA ALTERNATIVA -->
    <div class="card mb-3">
        <div class="card-header" style="background: #1c2437; color: white;">
            <i class="fas fa-truck me-1"></i> Proveedor de Agua Alternativa (Carrotanque)
        </div>
        <div class="card-body">
            <div class="mb-3">
                <label class="form-label fw-bold">Nombre y contacto del proveedor de carrotanque</label>
                <textarea name="empresa_carrotanque" class="form-control" rows="3"
                          placeholder="Ej: Carrotanques del Valle S.A.S. — Tel: 312 000 0000 — Contacto: Pedro Gómez"><?= esc($empresaCarrotanque) ?></textarea>
                <small class="text-muted">Proveedor que suministra agua en caso de corte del servicio.</small>
            </div>
            <div class="mb-3">
                <label class="form-label fw-bold">Capacidad de reserva</label>
                <input type="text" name="capacidad_reserva" class="form-control"
                       value="<?= esc($capacidadReserva) ?>"
                       placeholder="Ej: 10.000 litros — tanques de almacenamiento en piso 1 y piso 8">
                <small class="text-muted">Capacidad total de almacenamiento de agua del conjunto.</small>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-3">
        <div class="card-body">
            <p class="text-muted mb-0" style="font-size: 13px;">
                ...
                Al finalizar se generará automáticamente el documento <strong>FT-SST-234</strong>
                (Plan de Contingencia Sin Suministro de Agua) con el protocolo de actuación completo.
            </p>
        </div>
    </div>
...
function restoreFromLocal(data) {
    ...
    if (data.empresa_carrotanque) document.querySelector('[name="empresa_carrotanque"]').value = data.empresa_carrotanque;
    if (data.capacidad_reserva) document.querySelector('[name="capacidad_reserva"]').value = data.capacidad_reserva;
...
initAutosave({
    formId: 'aguaForm',
    storeUrl: base_url('/inspecciones/contingencia-agua/store'),
    updateUrlBase: base_url('/inspecciones/contingencia-agua/update/'),
    editUrlBase: base_url('/inspecciones/contingencia-agua/edit/'),
