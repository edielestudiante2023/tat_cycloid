...
$action = $isEdit ? base_url('/inspecciones/kpi-limpieza/update/') . $inspeccion['id'] : base_url('/inspecciones/kpi-limpieza/store');
...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a href="<?= base_url('/inspecciones/kpi-limpieza') ?>" class="btn btn-sm btn-outline-secondary">
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        btnFinalizar.addEventListener('click', function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = '<?= base_url('/inspecciones/' . esc($slug ?? 'kpi-limpieza') . '/finalizar-grupo/' . ($inspeccion['id'] ?? '')) ?>';
                }
            });
        });
    }
    ...
    initAutosave({
        ...
        storeUrl: base_url('/inspecciones/kpi-limpieza/store'),
        updateUrlBase: base_url('/inspecciones/kpi-limpieza/update/'),
        editUrlBase: base_url('/inspecciones/kpi-limpieza/edit/'),
...
(function() {
    ...
    var slug = '<?= esc($slug ?? 'kpi-limpieza') ?>';
    ...
    setTimeout(function() {
        ...
        Swal.fire({
            ...
            confirmButtonColor: '#28a745',
            denyButtonColor: '#ee6c21',
        }).then(function(result) {
            if (result.isConfirmed && currentId) {
                window.location.href = '<?= base_url('/inspecciones/' . ($slug ?? 'kpi-limpieza') . '/finalizar-grupo/') ?>' + currentId;
            } else if (result.isDenied) {
                window.location.href = '<?= base_url('/inspecciones/' . ($slug ?? 'kpi-limpieza') . '/create/') ?>' + savedCliente;
            }
        });
    }, 500);
})();
