...
<div class="card mb-3">
    ...
    <div class="card-body p-3">
        <table class="table table-sm table-bordered mb-0" style="font-size:13px;">
            ...
            <?php
                $cfg = ($indicadorConfig[$inspeccion['indicador']] ?? null);
            ?>
            <?php if ($cfg): ?>
...
<a href="<?= base_url('/inspecciones/kpi-limpieza/pdf/') ?><?= $inspeccion['id'] ?>" class="btn btn-pwa btn-pwa-primary mb-4" target="_blank">
    <i class="fas fa-file-pdf me-2"></i>Ver PDF
</a>
<?php endif; ?>
    <?php if ($inspeccion['estado'] === 'completo'): ?>
    <a href="<?= base_url('/inspecciones/kpi-limpieza/regenerar/') ?><?= $inspeccion['id'] ?>" class="btn btn-pwa btn-pwa-outline" onclick="return confirm('¿Regenerar el PDF con la plantilla actual?')">
        <i class="fas fa-sync-alt me-2"></i>Regenerar PDF
    </a>
    <a href="<?= base_url('/inspecciones/kpi-limpieza/enviar-email/') ?><?= $inspeccion['id'] ?>" class="btn btn-pwa btn-pwa-outline" onclick="return confirm('¿Enviar el PDF por email al cliente, consultor y consultor externo?')">
