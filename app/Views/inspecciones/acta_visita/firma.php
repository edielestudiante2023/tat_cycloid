...
function guardarFirma(tipo, paso) {
    ...
    if (!canvas || !canvas.hayDibujo) {
        Swal.fire({ icon: 'warning', title: 'Dibuje su firma primero', confirmButtonColor: '#ee6c21' });
        return;
    }
    if (!canvas.validarMinPixeles(100)) {
        Swal.fire({ icon: 'warning', title: 'La firma es muy pequena', text: 'Por favor firme con un trazo mas visible', confirmButtonColor: '#ee6c21' });
        return;
    }
    ...
    Swal.fire({
        ...
        .catch(async () => {
            ...
            try {
                ...
                Swal.fire({ icon: 'info', title: 'Guardado offline', html: 'Sin conexion. La firma se enviara automaticamente cuando vuelva el internet.', confirmButtonColor: '#ee6c21' });
