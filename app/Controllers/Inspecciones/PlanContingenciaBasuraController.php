...
class PlanContingenciaBasuraController extends BaseController
{
    ...
    public function list()
    {
        $inspecciones = $this->inspeccionModel
            ->select('tbl_plan_contingencia_basura.*, tbl_clientes.nombre_cliente, tbl_consultor.nombre_consultor')
            ->join('tbl_clientes', 'tbl_clientes.id_cliente = tbl_plan_contingencia_basura.id_cliente', 'left')
            ->join('tbl_consultor', 'tbl_consultor.id_consultor = tbl_plan_contingencia_basura.id_consultor', 'left')
            ->orderBy('tbl_plan_contingencia_basura.fecha_programa', 'DESC')
            ->findAll();

        return view('inspecciones/layout_pwa', [
            'title'   => 'Plan de Contingencia — Recolección de Basuras',
            'content' => view('inspecciones/contingencia-basura/list', [
    ...
    {
        return view('inspecciones/layout_pwa', [
            'title'   => 'Nuevo Plan Contingencia Basura',
            'content' => view('inspecciones/contingencia-basura/form', [
                'inspeccion' => null,
                'idCliente'  => $idCliente,
            ]),
        ]);
    ...
    public function store()
    {
        ...
        $data = [
            ...
            'empresa_aseo'             => $this->request->getPost('empresa_aseo'),
            'horario_recoleccion_actual' => $this->request->getPost('horario_recoleccion_actual'),
            'estado'                   => 'borrador',
        ];
        ...
        return redirect()->to("/inspecciones/contingencia-basura/edit/{$id}")->with('msg', 'Plan guardado como borrador.');
    }
    ...
    public function edit($id)
    {
        ...
        if (!$inspeccion) {
            return redirect()->to('/inspecciones/contingencia-basura')->with('error', 'Registro no encontrado.');
        }

        return view('inspecciones/layout_pwa', [
            'title'   => 'Editar Plan Contingencia Basura',
            'content' => view('inspecciones/contingencia-basura/form', [
    ...
    public function update($id)
    {
        ...
        if (!$inspeccion) {
            ...
            return redirect()->to('/inspecciones/contingencia-basura')->with('error', 'No se puede editar.');
        }
        ...
        return redirect()->to("/inspecciones/contingencia-basura/edit/{$id}")->with('msg', 'Cambios guardados.');
    }
    ...
    public function view($id)
    {
        ...
        return view('inspecciones/layout_pwa', [
            'title'   => 'Plan de Contingencia — Recolección de Basuras',
            'content' => view('inspecciones/contingencia-basura/view', [
                'inspeccion' => $inspeccion,
    ...
    public function finalizar($id)
    {
        ...
        $emailResult = InspeccionEmailNotifier::enviar(
            ...
            'PLAN DE CONTINGENCIA — RECOLECCIÓN DE BASURAS',
            $inspeccion['fecha_programa'],
            $pdfPath,
            (int) $inspeccion['id'],
            'PlanContingenciaBasura',
        ...
        return redirect()->to("/inspecciones/contingencia-basura/view/{$id}")->with('msg', $msg);
    }
    ...
    public function generatePdf($id)
    {
        ...
        return $this->response
            ...
            ->setHeader('Content-Disposition', 'inline; filename="contingencia_basura_' . $id . '.pdf"')
            ->setBody($pdfContent);
    }
    ...
    public function regenerarPdf($id)
    {
        ...
        if (!$inspeccion || ($inspeccion['estado'] ?? '') !== 'completo') {
            return redirect()->to('/inspecciones/contingencia-basura')->with('error', 'Solo se puede regenerar un registro finalizado.');
        }
        ...
        return redirect()->to("/inspecciones/contingencia-basura/view/{$id}")->with('msg', 'PDF regenerado exitosamente.');
    }
    ...
    public function enviarEmail($id)
    {
        ...
        if (!$inspeccion || $inspeccion['estado'] !== 'completo' || empty($inspeccion['ruta_pdf'])) {
            return redirect()->to("/inspecciones/contingencia-basura/view/{$id}")->with('error', 'El documento debe estar finalizado con PDF para enviar email.');
        }
        ...
        if ($result['success']) {
            return redirect()->to("/inspecciones/contingencia-basura/view/{$id}")->with('msg', $result['message']);
        }
        return redirect()->to("/inspecciones/contingencia-basura/view/{$id}")->with('error', $result['error']);
    }
    ...
    public function documento()
    {
        ...
        $html = view('inspecciones/contingencia-basura/pdf', $data);
    ...
    private function generarPdfInterno($id): string
    {
        ...
        $pdfDir = 'uploads/inspecciones/contingencia-basura/pdfs/';
