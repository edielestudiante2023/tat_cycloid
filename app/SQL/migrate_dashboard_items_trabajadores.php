<?php
/**
 * TAT Fase 4.1 — Inserta item "Trabajadores" en dashboard_items del consultor.
 *
 * Uso:
 *   php app/SQL/migrate_dashboard_items_trabajadores.php local
 *   DB_PROD_PASS=xxxxx php app/SQL/migrate_dashboard_items_trabajadores.php production
 */
...
$check = $m->query("SELECT id FROM dashboard_items WHERE accion_url = 'trabajadores/seleccionar-cliente'");
if ($check && $check->num_rows > 0) {
    $row = $check->fetch_assoc();
    echo "Item ya existe (id={$row['id']}), actualizando campos...\n";
    ...
    $sql = "INSERT INTO dashboard_items
        ...
        VALUES
        ('consultor', 'modulo', 'Trabajadores',
         'Gestión de trabajadores del establecimiento comercial (datos, afiliación, manipulación alimentos, dotación)',
         'trabajadores/seleccionar-cliente', 15, 'Gestión Clientes',
         'fa-users', 'linear-gradient(135deg, #6f42c1 0%, #8e5ed8 100%)',
         1, 1, NOW())";
    if ($m->query($sql)) {
        echo "Item insertado (id=" . $m->insert_id . ").\n";
    } else {
        die("ERR insert: " . $m->error . "\n");
    }
}
echo "OK.\n";
